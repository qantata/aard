 ...
 */
const prepareMatches = <Params extends { [K in keyof Params]?: string }>(
...
export const createRouter = (routes: RouteConfig[]) => {
  ...
  const cleanup = history.listen((location, action) => {
    ...
    if (newLocation.pathname === currentEntry.location.pathname) {
      return;
    }

    const matches = matchRoute(routes, newLocation);
  ...
  const context = {
    ...
    preloadCode(pathname: string) {
      ...
      // @ts-ignore TypeScript doesn't know about load method
